...
let s32_encode (n : int64) : t =
  let rec s32_encode ~tid n =
    match n with
    | 0L ->
        tid
    | n ->
...
let s32_decode (s : t) : int64 =
  let rec s32_decode ~(n : int64) (s : string) =
    match s with
...
let of_timestamp_us ?(clockid = Random.int_in_range ~min:0 ~max:1023)
    (timestamp : int64) : t =
  ...
  in
  pad (s32_encode timestamp) 11 ^ pad (s32_encode @@ Int64.of_int clockid) 2

let of_timestamp_ms ?(clockid = Random.int_in_range ~min:0 ~max:1023)
    (timestamp : int64) : t =
  of_timestamp_us
    (Int64.add (Int64.mul timestamp 1000L) (Random.int64 1000L))
    ~clockid

let to_timestamp_us (tid : t) : int64 * int =
  ensure_valid tid ;
  let timestamp = s32_decode (String.sub tid 0 11) in
  let clockid = Int64.to_int @@ s32_decode (String.sub tid 11 2) in
  (timestamp, clockid)
...
let now () : t =
  ...
  Time_stamp_counter.Calibrator.calibrate calibrator ;
  Time_stamp_counter.now ()
  |> Time_stamp_counter.to_time ~calibrator
  |> Time_float_unix.to_span_since_epoch |> Time_float_unix.Span.to_us
  |> Int64.of_float
