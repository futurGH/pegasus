...
module Verifiers = struct
  open struct
    ...
    let is_service_auth_token req =
      match parse_header req "Bearer" with
      | Error _ ->
          false
      | Ok token -> (
        ...
        | Error _ ->
            false
        | Ok (_header, payload) -> (
          match Yojson.Safe.Util.member "lxm" payload with
          | `Null ->
              false
          | _ ->
              true ) )
  ...
  let user_service_auth : verifier =
    let check_actor_status did db =
      match%lwt Data_store.get_actor_by_identifier did db with
      | Some {deactivated_at= None; _} ->
          Lwt.return_ok (Access {did})
      | Some {deactivated_at= Some _; _} ->
  ...
  let authorization : verifier =
   fun ctx ->
    ...
    | Some ("Basic" :: _) ->
        admin ctx
    | Some ("Bearer" :: _) ->
        (* Check if it's a service auth token (has lxm claim) *)
        if is_service_auth_token ctx.req then user_service_auth ctx
        else bearer ctx
    | Some ("DPoP" :: _) ->
        oauth ctx
    | _ -> (
