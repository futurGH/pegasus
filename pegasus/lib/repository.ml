...
let apply_writes (t : t) (writes : repo_write list) (swap_commit : Cid.t option)
  ...
  with_write_lock t.did (fun () ->
      Dream.debug (fun l -> l "lock acquired") ;
      ...
      let%lwt prev_commit =
        ...
        | None ->
            failwith ("failed to retrieve commit for " ^ t.did)
      in
      Dream.debug (fun l -> l "commit retrieved") ;
      ...
      let%lwt results =
        Lwt_list.map_s
          ...
          writes
      in
      Dream.debug (fun l -> l "writes processed") ;
      let new_mst = !mst in
      let%lwt new_commit =
        put_commit t new_mst.root ~previous:(Some prev_commit)
      in
      Dream.debug (fun l -> l "commit inserted") ;
      ...
      let%lwt proof_blocks =
        ...
        | Error err ->
            raise err
      in
      Dream.debug (fun l -> l "proof generated") ;
      let block_stream =
