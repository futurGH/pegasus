...
module Main = struct
  ...
  type input = {name: string; privileged: bool option [@default None]}
  [@@deriving yojson {strict= false}]

  type output = app_password [@@deriving yojson {strict= false}]

  let call ~name ?privileged (client : Hermes.client) : output Lwt.t =
    let params = () in
