...
type record_blob = {cid: string; record_uri: string [@key "recordUri"]}
...
module Main = struct
  ...
  type output = {cursor: string option [@default None]; blobs: record_blob list}
  [@@deriving yojson {strict= false}]

  let call ?limit ?cursor (client : Hermes.client) : output Lwt.t =
