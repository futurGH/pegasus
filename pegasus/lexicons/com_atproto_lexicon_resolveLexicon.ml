...
module Main = struct
  ...
  type params = {nsid: string} [@@deriving yojson {strict= false}]

  type output =
    {cid: string; schema: Com_atproto_lexicon_schema.main; uri: string}
  [@@deriving yojson {strict= false}]

  let call ~nsid (client : Hermes.client) : output Lwt.t =
