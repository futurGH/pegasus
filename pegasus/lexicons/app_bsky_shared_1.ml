...
let actor_embed_to_yojson = function
  | ExternalView v -> (
    ...
    | `Assoc fields ->
        `Assoc (("$type", `String "app.bsky.embed.external#view") :: fields)
    | other ->
        other )
  | Unknown j ->
      j
...
type list_viewer_state =
  {muted: bool option [@default None]; blocked: string option [@default None]}
[@@deriving yojson {strict= false}]
...
let list_purpose_of_yojson = function
  | `String s ->
      Ok s
  | _ ->
      Error "list_purpose: expected string"
...
type profile_associated_chat = {allow_incoming: string [@key "allowIncoming"]}
[@@deriving yojson {strict= false}]
...
type profile_associated =
  ...
  ; activity_subscription: profile_associated_activity_subscription option
        [@key "activitySubscription"] [@default None] }
[@@deriving yojson {strict= false}]
...
and actor_viewer_state =
  ...
  ; blocking_by_list: list_view_basic option
        [@key "blockingByList"] [@default None]
  ...
  ; known_followers: known_followers option
        [@key "knownFollowers"] [@default None]
  ; activity_subscription:
      App_bsky_notification_defs.activity_subscription option
        [@key "activitySubscription"] [@default None] }

and known_followers = {count: int; followers: profile_view_basic list}
...
let rec profile_view_basic_of_yojson json =
  ...
  try
    ...
    let associated =
      json |> member "associated"
      |> to_option (fun x ->
          match profile_associated_of_yojson x with Ok v -> Some v | _ -> None )
      |> Option.join
    in
    let viewer =
      json |> member "viewer"
      |> to_option (fun x ->
          match actor_viewer_state_of_yojson x with Ok v -> Some v | _ -> None )
      |> Option.join
    in
    let labels =
      json |> member "labels"
      |> to_option (fun j ->
          ...
          |> List.filter_map (fun x ->
              match Com_atproto_label_defs.label_of_yojson x with
              | Ok v ->
                  Some v
              | _ ->
                  None ) )
    ...
    let verification =
      json |> member "verification"
      |> to_option (fun x ->
          match verification_state_of_yojson x with Ok v -> Some v | _ -> None )
      |> Option.join
    in
    let status =
      json |> member "status"
      |> to_option (fun x ->
          match status_view_of_yojson x with Ok v -> Some v | _ -> None )
      |> Option.join
    in
    Ok
      { did
...
and actor_viewer_state_of_yojson json =
  ...
  try
    ...
    let muted_by_list =
      json |> member "mutedByList"
      |> to_option (fun x ->
          match list_view_basic_of_yojson x with Ok v -> Some v | _ -> None )
      |> Option.join
    in
    let blocked_by = json |> member "blockedBy" |> to_option to_bool in
    let blocking = json |> member "blocking" |> to_option to_string in
    let blocking_by_list =
      json |> member "blockingByList"
      |> to_option (fun x ->
          match list_view_basic_of_yojson x with Ok v -> Some v | _ -> None )
      |> Option.join
    in
    ...
    let known_followers =
      json |> member "knownFollowers"
      |> to_option (fun x ->
          match known_followers_of_yojson x with Ok v -> Some v | _ -> None )
      |> Option.join
    in
    let activity_subscription =
      json
      |> member "activitySubscription"
      |> to_option (fun x ->
          match
            App_bsky_notification_defs.activity_subscription_of_yojson x
          with
          | Ok v ->
              Some v
          | _ ->
...
and known_followers_of_yojson json =
  ...
  try
    ...
    let followers =
      json |> member "followers"
      |> fun j ->
      to_list j
      |> List.filter_map (fun x ->
          match profile_view_basic_of_yojson x with Ok v -> Some v | _ -> None )
    in
    Ok {count; followers}
  with e -> Error (Printexc.to_string e)
...
and profile_view_basic_to_yojson (r : profile_view_basic) =
  `Assoc
    ...
    ; ( "displayName"
      , match r.display_name with
        | Some v ->
            (fun s -> `String s) v
        | None ->
            `Null )
    ; ( "avatar"
      , match r.avatar with Some v -> (fun s -> `String s) v | None -> `Null )
    ; ( "associated"
      , match r.associated with
        | Some v ->
            profile_associated_to_yojson v
        | None ->
            `Null )
    ; ( "viewer"
      , match r.viewer with
        | Some v ->
            actor_viewer_state_to_yojson v
        | None ->
            `Null )
    ; ( "labels"
      , match r.labels with
        | Some v ->
            (fun l -> `List (List.map Com_atproto_label_defs.label_to_yojson l))
              v
        | None ->
            `Null )
    ; ( "createdAt"
      , match r.created_at with
        | Some v ->
            (fun s -> `String s) v
        | None ->
            `Null )
    ; ( "verification"
      , match r.verification with
        | Some v ->
            verification_state_to_yojson v
        | None ->
            `Null )
    ; ( "status"
      , match r.status with Some v -> status_view_to_yojson v | None -> `Null )
    ]

and actor_viewer_state_to_yojson (r : actor_viewer_state) =
  `Assoc
    [ ( "muted"
      , match r.muted with Some v -> (fun b -> `Bool b) v | None -> `Null )
    ; ( "mutedByList"
      , match r.muted_by_list with
        | Some v ->
            list_view_basic_to_yojson v
        | None ->
            `Null )
    ; ( "blockedBy"
      , match r.blocked_by with Some v -> (fun b -> `Bool b) v | None -> `Null
      )
    ; ( "blocking"
      , match r.blocking with Some v -> (fun s -> `String s) v | None -> `Null
      )
    ; ( "blockingByList"
      , match r.blocking_by_list with
        | Some v ->
            list_view_basic_to_yojson v
        | None ->
            `Null )
    ; ( "following"
      , match r.following with
        | Some v ->
            (fun s -> `String s) v
        | None ->
            `Null )
    ; ( "followedBy"
      , match r.followed_by with
        | Some v ->
            (fun s -> `String s) v
        | None ->
            `Null )
    ; ( "knownFollowers"
      , match r.known_followers with
        | Some v ->
            known_followers_to_yojson v
        | None ->
            `Null )
    ; ( "activitySubscription"
      , match r.activity_subscription with
        | Some v ->
            App_bsky_notification_defs.activity_subscription_to_yojson v
        | None ->
            `Null ) ]
...
and known_followers_to_yojson (r : known_followers) =
  `Assoc
    ...
    ; ( "followers"
      , (fun l -> `List (List.map profile_view_basic_to_yojson l)) r.followers
      ) ]
...
type starter_pack_view_basic =
  ...
  ; joined_all_time_count: int option
        [@key "joinedAllTimeCount"] [@default None]
  ; labels: Com_atproto_label_defs.label list option [@default None]
...
type profile_view_detailed =
  ...
  ; associated: profile_associated option [@default None]
  ; joined_via_starter_pack: starter_pack_view_basic option
        [@key "joinedViaStarterPack"] [@default None]
  ; indexed_at: string option [@key "indexedAt"] [@default None]
  ; created_at: string option [@key "createdAt"] [@default None]
  ; viewer: actor_viewer_state option [@default None]
  ; labels: Com_atproto_label_defs.label list option [@default None]
  ; pinned_post: Com_atproto_repo_strongRef.main option
        [@key "pinnedPost"] [@default None]
  ; verification: verification_state option [@default None]
  ; status: status_view option [@default None] }
[@@deriving yojson {strict= false}]
...
let postgate_embedding_rules_item_to_yojson = function
  | PostgateDisableRule v -> (
    ...
    | `Assoc fields ->
        `Assoc
          (("$type", `String "app.bsky.feed.postgate#disableRule") :: fields)
    | other ->
        other )
  | Unknown j ->
      j
...
let threadgate_allow_rules_item_to_yojson = function
  | ThreadgateMentionRule v -> (
    ...
    | `Assoc fields ->
        `Assoc
          (("$type", `String "app.bsky.feed.threadgate#mentionRule") :: fields)
    | other ->
        other )
  | ThreadgateFollowerRule v -> (
    ...
    | `Assoc fields ->
        `Assoc
          (("$type", `String "app.bsky.feed.threadgate#followerRule") :: fields)
    | other ->
        other )
  | ThreadgateFollowingRule v -> (
    ...
    | `Assoc fields ->
        `Assoc
          (("$type", `String "app.bsky.feed.threadgate#followingRule") :: fields)
    | other ->
        other )
  | ThreadgateListRule v -> (
    match App_bsky_feed_threadgate.list_rule_to_yojson v with
    | `Assoc fields ->
        `Assoc (("$type", `String "app.bsky.feed.threadgate#listRule") :: fields)
    | other ->
        other )
  | Unknown j ->
      j
...
type post_interaction_settings_pref =
  { threadgate_allow_rules: threadgate_allow_rules_item list option
        [@key "threadgateAllowRules"] [@default None]
  ; postgate_embedding_rules: postgate_embedding_rules_item list option
        [@key "postgateEmbeddingRules"] [@default None] }
[@@deriving yojson {strict= false}]

type labeler_pref_item = {did: string} [@@deriving yojson {strict= false}]
...
type bsky_app_progress_guide = {guide: string}
[@@deriving yojson {strict= false}]

type bsky_app_state_pref =
  { active_progress_guide: bsky_app_progress_guide option
        [@key "activeProgressGuide"] [@default None]
...
let muted_word_target_of_yojson = function
  | `String s ->
      Ok s
  | _ ->
      Error "muted_word_target: expected string"
...
type interests_pref = {tags: string list} [@@deriving yojson {strict= false}]

type thread_view_pref =
  { sort: string option [@default None]
  ; prioritize_followed_users: bool option
        [@key "prioritizeFollowedUsers"] [@default None] }
[@@deriving yojson {strict= false}]
...
type feed_view_pref =
  ...
  ; hide_replies_by_unfollowed: bool option
        [@key "hideRepliesByUnfollowed"] [@default None]
  ; hide_replies_by_like_count: int option
        [@key "hideRepliesByLikeCount"] [@default None]
...
type saved_feed =
  {id: string; type_: string [@key "type"]; value: string; pinned: bool}
[@@deriving yojson {strict= false}]

type saved_feeds_pref_v2 = {items: saved_feed list}
[@@deriving yojson {strict= false}]
...
type adult_content_pref = {enabled: bool} [@@deriving yojson {strict= false}]
...
let preferences_item_to_yojson = function
  | AdultContentPref v -> (
    ...
    | `Assoc fields ->
        `Assoc
          (("$type", `String "app.bsky.actor.defs#adultContentPref") :: fields)
    | other ->
        other )
  | ContentLabelPref v -> (
    ...
    | `Assoc fields ->
        `Assoc
          (("$type", `String "app.bsky.actor.defs#contentLabelPref") :: fields)
    | other ->
        other )
  | SavedFeedsPref v -> (
  ...
  | SavedFeedsPrefV2 v -> (
    ...
    | `Assoc fields ->
        `Assoc
          (("$type", `String "app.bsky.actor.defs#savedFeedsPrefV2") :: fields)
    | other ->
        other )
  ...
  | FeedViewPref v -> (
    ...
    | `Assoc fields ->
        `Assoc (("$type", `String "app.bsky.actor.defs#feedViewPref") :: fields)
    | other ->
        other )
  | ThreadViewPref v -> (
    ...
    | `Assoc fields ->
        `Assoc
          (("$type", `String "app.bsky.actor.defs#threadViewPref") :: fields)
    | other ->
        other )
  | InterestsPref v -> (
    match interests_pref_to_yojson v with
    | `Assoc fields ->
        `Assoc (("$type", `String "app.bsky.actor.defs#interestsPref") :: fields)
    | other ->
        other )
  | MutedWordsPref v -> (
    ...
    | `Assoc fields ->
        `Assoc
          (("$type", `String "app.bsky.actor.defs#mutedWordsPref") :: fields)
    | other ->
        other )
  | HiddenPostsPref v -> (
    ...
    | `Assoc fields ->
        `Assoc
          (("$type", `String "app.bsky.actor.defs#hiddenPostsPref") :: fields)
    | other ->
        other )
  | BskyAppStatePref v -> (
    ...
    | `Assoc fields ->
        `Assoc
          (("$type", `String "app.bsky.actor.defs#bskyAppStatePref") :: fields)
    | other ->
        other )
  | LabelersPref v -> (
    match labelers_pref_to_yojson v with
    | `Assoc fields ->
        `Assoc (("$type", `String "app.bsky.actor.defs#labelersPref") :: fields)
    | other ->
        other )
  | PostInteractionSettingsPref v -> (
    ...
    | `Assoc fields ->
        `Assoc
          ( ("$type", `String "app.bsky.actor.defs#postInteractionSettingsPref")
          :: fields )
    | other ->
        other )
  | VerificationPrefs v -> (
    ...
    | `Assoc fields ->
        `Assoc
          (("$type", `String "app.bsky.actor.defs#verificationPrefs") :: fields)
    | other ->
        other )
  | Unknown j ->
      j
...
let preferences_of_yojson json =
  ...
  Ok
    ( to_list json
    |> List.filter_map (fun x ->
        match preferences_item_of_yojson x with Ok v -> Some v | _ -> None ) )

let preferences_to_yojson l = `List (List.map preferences_item_to_yojson l)

type record_main = {record: Com_atproto_repo_strongRef.main}
[@@deriving yojson {strict= false}]

type labeler_viewer_state = {like: string option [@default None]}
[@@deriving yojson {strict= false}]
...
type list_view =
  ...
  ; purpose: list_purpose
  ; description: string option [@default None]
  ; description_facets: App_bsky_richtext_facet.main list option
        [@key "descriptionFacets"] [@default None]
  ; avatar: string option [@default None]
  ; list_item_count: int option [@key "listItemCount"] [@default None]
  ; labels: Com_atproto_label_defs.label list option [@default None]
  ; viewer: list_viewer_state option [@default None]
...
type generator_viewer_state = {like: string option [@default None]}
[@@deriving yojson {strict= false}]
...
type generator_view =
  ...
  ; accepts_interactions: bool option
        [@key "acceptsInteractions"] [@default None]
  ; labels: Com_atproto_label_defs.label list option [@default None]
...
type blocked_author =
  {did: string; viewer: actor_viewer_state option [@default None]}
[@@deriving yojson {strict= false}]

type view_blocked = {uri: string; blocked: bool; author: blocked_author}
[@@deriving yojson {strict= false}]

type view_not_found = {uri: string; not_found: bool [@key "notFound"]}
[@@deriving yojson {strict= false}]
...
let record_with_media_media_to_yojson = function
  | Images v -> (
    ...
    | `Assoc fields ->
        `Assoc (("$type", `String "app.bsky.embed.images") :: fields)
    | other ->
        other )
  | Video v -> (
    match App_bsky_embed_video.main_to_yojson v with
    | `Assoc fields ->
        `Assoc (("$type", `String "app.bsky.embed.video") :: fields)
    | other ->
        other )
  | External v -> (
    match App_bsky_embed_external.main_to_yojson v with
    | `Assoc fields ->
        `Assoc (("$type", `String "app.bsky.embed.external") :: fields)
    | other ->
        other )
  | Unknown j ->
      j
...
and record_with_media_view =
  {record: record_view; media: record_with_media_media}
...
and view_record_of_yojson json =
  ...
  try
    ...
    let author =
      json |> member "author" |> profile_view_basic_of_yojson |> Result.get_ok
    in
    ...
    let embeds =
      ...
      |> to_option (fun j ->
          to_list j
          |> List.filter_map (fun x ->
              match embeds_item_of_yojson x with Ok v -> Some v | _ -> None ) )
    in
...
and record_with_media_view_of_yojson json =
  ...
  try
    let record =
      json |> member "record" |> record_view_of_yojson |> Result.get_ok
    in
    let media =
      json |> member "media" |> record_with_media_media_of_yojson
      |> Result.get_ok
    in
    Ok {record; media}
  with e -> Error (Printexc.to_string e)
...
and embeds_item_to_yojson = function
  | ImagesView v -> (
    ...
    | `Assoc fields ->
        `Assoc (("$type", `String "app.bsky.embed.images#view") :: fields)
    | other ->
        other )
  | VideoView v -> (
    match App_bsky_embed_video.view_to_yojson v with
    | `Assoc fields ->
        `Assoc (("$type", `String "app.bsky.embed.video#view") :: fields)
    | other ->
        other )
  | ExternalView v -> (
    match App_bsky_embed_external.view_to_yojson v with
  ...
  | RecordView v -> (
    ...
    | `Assoc fields ->
        `Assoc (("$type", `String "app.bsky.embed.record#view") :: fields)
    | other ->
        other )
  | RecordWithMediaView v -> (
    ...
    | `Assoc fields ->
        `Assoc
          (("$type", `String "app.bsky.embed.recordWithMedia#view") :: fields)
    | other ->
        other )
  | Unknown j ->
      j
...
and record_to_yojson = function
  | ViewRecord v -> (
    ...
    | `Assoc fields ->
        `Assoc (("$type", `String "app.bsky.embed.record#viewRecord") :: fields)
    | other ->
        other )
  | ViewNotFound v -> (
    ...
    | `Assoc fields ->
        `Assoc
          (("$type", `String "app.bsky.embed.record#viewNotFound") :: fields)
    | other ->
        other )
  | ViewBlocked v -> (
    match view_blocked_to_yojson v with
    | `Assoc fields ->
        `Assoc (("$type", `String "app.bsky.embed.record#viewBlocked") :: fields)
    | other ->
        other )
  | ViewDetached v -> (
    ...
    | `Assoc fields ->
        `Assoc
          (("$type", `String "app.bsky.embed.record#viewDetached") :: fields)
    | other ->
        other )
  | DefsGeneratorView v -> (
    match generator_view_to_yojson v with
    | `Assoc fields ->
        `Assoc (("$type", `String "app.bsky.feed.defs#generatorView") :: fields)
    | other ->
        other )
  | DefsListView v -> (
    match list_view_to_yojson v with
    | `Assoc fields ->
        `Assoc (("$type", `String "app.bsky.graph.defs#listView") :: fields)
    | other ->
        other )
  | DefsLabelerView v -> (
    match labeler_view_to_yojson v with
    | `Assoc fields ->
        `Assoc (("$type", `String "app.bsky.labeler.defs#labelerView") :: fields)
    | other ->
        other )
  | DefsStarterPackViewBasic v -> (
    ...
    | `Assoc fields ->
        `Assoc
          ( ("$type", `String "app.bsky.graph.defs#starterPackViewBasic")
          :: fields )
    | other ->
        other )
  | Unknown j ->
...
and record_view_to_yojson (r : record_view) =
  `Assoc [("record", record_to_yojson r.record)]
    ...
    ; ( "labels"
      , match r.labels with
        ...
        | None ->
            `Null )
    ; ( "replyCount"
      , match r.reply_count with Some v -> (fun i -> `Int i) v | None -> `Null
      )
    ; ( "repostCount"
      , match r.repost_count with
        | Some v ->
            (fun i -> `Int i) v
        | None ->
            `Null )
    ; ( "likeCount"
      , match r.like_count with Some v -> (fun i -> `Int i) v | None -> `Null )
    ; ( "quoteCount"
      , match r.quote_count with Some v -> (fun i -> `Int i) v | None -> `Null
      )
    ; ( "embeds"
      , match r.embeds with
        | Some v ->
            (fun l -> `List (List.map embeds_item_to_yojson l)) v
        | None ->
            `Null )
...
type record_with_media_main =
  {record: record_main; media: record_with_media_media}
[@@deriving yojson {strict= false}]
...
type blocked_post = {uri: string; blocked: bool; author: blocked_author}
[@@deriving yojson {strict= false}]

type not_found_post = {uri: string; not_found: bool [@key "notFound"]}
[@@deriving yojson {strict= false}]
...
let feed_parent_to_yojson = function
  | PostView v -> (
    ...
    | `Assoc fields ->
        `Assoc (("$type", `String "app.bsky.feed.defs#postView") :: fields)
    | other ->
        other )
  | NotFoundPost v -> (
    match not_found_post_to_yojson v with
    | `Assoc fields ->
        `Assoc (("$type", `String "app.bsky.feed.defs#notFoundPost") :: fields)
    | other ->
        other )
  | BlockedPost v -> (
    match blocked_post_to_yojson v with
    | `Assoc fields ->
        `Assoc (("$type", `String "app.bsky.feed.defs#blockedPost") :: fields)
    | other ->
        other )
  | Unknown j ->
      j
...
type reply_ref =
  { root: feed_parent
  ; parent: feed_parent
  ; grandparent_author: profile_view_basic option
        [@key "grandparentAuthor"] [@default None] }
[@@deriving yojson {strict= false}]
...
let feed_reason_to_yojson = function
  | ReasonRepost v -> (
    ...
    | `Assoc fields ->
        `Assoc (("$type", `String "app.bsky.feed.defs#reasonRepost") :: fields)
    | other ->
        other )
  | ReasonPin v -> (
    match reason_pin_to_yojson v with
    | `Assoc fields ->
        `Assoc (("$type", `String "app.bsky.feed.defs#reasonPin") :: fields)
    | other ->
        other )
  | Unknown j ->
      j
...
and thread_view_post_of_yojson json =
  ...
  try
    ...
    let parent =
      json |> member "parent"
      |> to_option (fun x ->
          match replies_item_of_yojson x with Ok v -> Some v | _ -> None )
      |> Option.join
    in
    let replies =
      json |> member "replies"
    ...
    in
    let thread_context =
      json |> member "threadContext"
      |> to_option (fun x ->
          match thread_context_of_yojson x with Ok v -> Some v | _ -> None )
      |> Option.join
    in
...
and replies_item_to_yojson = function
  | ThreadViewPost v -> (
    ...
    | `Assoc fields ->
        `Assoc (("$type", `String "app.bsky.feed.defs#threadViewPost") :: fields)
    | other ->
        other )
  | NotFoundPost v -> (
    match not_found_post_to_yojson v with
...
and thread_view_post_to_yojson (r : thread_view_post) =
  `Assoc
    ...
    ; ( "parent"
      , match r.parent with Some v -> replies_item_to_yojson v | None -> `Null
      )
    ; ( "replies"
      , match r.replies with
        | Some v ->
            (fun l -> `List (List.map replies_item_to_yojson l)) v
        | None ->
            `Null )
    ; ( "threadContext"
      , match r.thread_context with
        | Some v ->
            thread_context_to_yojson v
        | None ->
            `Null ) ]
...
type skeleton_reason_repost = {repost: string}
[@@deriving yojson {strict= false}]
...
type list_item_view = {uri: string; subject: profile_view}
[@@deriving yojson {strict= false}]
...
type starter_pack_view =
  ...
  ; list_items_sample: list_item_view list option
        [@key "listItemsSample"] [@default None]
...
type not_found_actor = {actor: string; not_found: bool [@key "notFound"]}
[@@deriving yojson {strict= false}]
...
type labeler_policies =
  ...
  ; label_value_definitions:
      Com_atproto_label_defs.label_value_definition list option
        [@key "labelValueDefinitions"] [@default None] }
[@@deriving yojson {strict= false}]
...
type labeler_view_detailed =
  ...
  ; indexed_at: string [@key "indexedAt"]
  ; labels: Com_atproto_label_defs.label list option [@default None]
  ; reason_types: Com_atproto_moderation_defs.reason_type list option
        [@key "reasonTypes"] [@default None]
  ; subject_types: Com_atproto_moderation_defs.subject_type list option
        [@key "subjectTypes"] [@default None]
  ; subject_collections: string list option
        [@key "subjectCollections"] [@default None] }
[@@deriving yojson {strict= false}]
