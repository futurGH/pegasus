...
module Main = struct
  ...
  let relationships_item_to_yojson = function
    | Relationship v -> (
      ...
      | `Assoc fields ->
          `Assoc
            (("$type", `String "app.bsky.graph.defs#relationship") :: fields)
      | other ->
          other )
    | NotFoundActor v -> (
      ...
      | `Assoc fields ->
          `Assoc
            (("$type", `String "app.bsky.graph.defs#notFoundActor") :: fields)
      | other ->
          other )
    | Unknown j ->
        j
  ...
  let call ~actor ?others (client : Hermes.client) : output Lwt.t =
