...
type value =
  ...
  | ThreadItemNoUnauthenticated of
      App_bsky_unspecced_defs.thread_item_no_unauthenticated
...
let value_to_yojson = function
  | ThreadItemPost v -> (
    ...
    | `Assoc fields ->
        `Assoc
          (("$type", `String "app.bsky.unspecced.defs#threadItemPost") :: fields)
    | other ->
        other )
  | ThreadItemNoUnauthenticated v -> (
    match
      App_bsky_unspecced_defs.thread_item_no_unauthenticated_to_yojson v
    with
    | `Assoc fields ->
        `Assoc
          ( ( "$type"
            , `String "app.bsky.unspecced.defs#threadItemNoUnauthenticated" )
          :: fields )
    | other ->
        other )
  | ThreadItemNotFound v -> (
    ...
    | `Assoc fields ->
        `Assoc
          ( ("$type", `String "app.bsky.unspecced.defs#threadItemNotFound")
          :: fields )
    | other ->
        other )
  | ThreadItemBlocked v -> (
...
type thread_item = {uri: string; depth: int; value: value}
...
module Main = struct
  ...
  let call ~anchor ?above ?below ?branching_factor ?prioritize_followed_users
      ?sort (client : Hermes.client) : output Lwt.t =
    let params : params =
      {anchor; above; below; branching_factor; prioritize_followed_users; sort}
    in
