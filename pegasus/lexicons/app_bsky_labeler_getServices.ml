...
module Main = struct
  ...
  let views_item_to_yojson = function
    | LabelerView v -> (
      ...
      | `Assoc fields ->
          `Assoc
            (("$type", `String "app.bsky.labeler.defs#labelerView") :: fields)
      | other ->
          other )
  ...
  type output = {views: views_item list} [@@deriving yojson {strict= false}]

  let call ~dids ?detailed (client : Hermes.client) : output Lwt.t =
