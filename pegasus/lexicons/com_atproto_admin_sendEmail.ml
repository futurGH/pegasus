...
module Main = struct
  ...
  type output = {sent: bool} [@@deriving yojson {strict= false}]

  let call ~recipient_did ~content ?subject ~sender_did ?comment
      (client : Hermes.client) : output Lwt.t =
    let params = () in
    let input =
      Some
        ( {recipient_did; content; subject; sender_did; comment}
        |> input_to_yojson )
    in
