...
type starter_pack_with_membership =
  ...
  ; list_item: App_bsky_graph_defs.list_item_view option
        [@key "listItem"] [@default None] }
...
module Main = struct
  ...
  type output =
    { cursor: string option [@default None]
    ; starter_packs_with_membership: starter_pack_with_membership list
          [@key "starterPacksWithMembership"] }
  [@@deriving yojson {strict= false}]

  let call ~actor ?limit ?cursor (client : Hermes.client) : output Lwt.t =
