...
type status_attr =
  {applied: bool; ref_: string option [@key "ref"] [@default None]}
...
type account_view =
  ...
  ; related_records: Yojson.Safe.t list option
        [@key "relatedRecords"] [@default None]
  ; indexed_at: string [@key "indexedAt"]
  ; invited_by: Com_atproto_server_defs.invite_code option
        [@key "invitedBy"] [@default None]
  ...
  ; threat_signatures: threat_signature list option
        [@key "threatSignatures"] [@default None] }
[@@deriving yojson {strict= false}]

type repo_ref = {did: string} [@@deriving yojson {strict= false}]
