...
type text_slice = {start: int; end_: int [@key "end"]}
[@@deriving yojson {strict= false}]

type entity = {index: text_slice; type_: string [@key "type"]; value: string}
[@@deriving yojson {strict= false}]
...
let labels_to_yojson = function
  | SelfLabels v -> (
    ...
    | `Assoc fields ->
        `Assoc (("$type", `String "com.atproto.label.defs#selfLabels") :: fields)
    | other ->
        other )
  | Unknown j ->
      j
...
let embed_to_yojson = function
  ...
  | Record v -> (
    ...
    | `Assoc fields ->
        `Assoc (("$type", `String "app.bsky.embed.record") :: fields)
    | other ->
        other )
  | RecordWithMedia v -> (
    match App_bsky_embed_recordWithMedia.main_to_yojson v with
    | `Assoc fields ->
        `Assoc (("$type", `String "app.bsky.embed.recordWithMedia") :: fields)
    | other ->
        other )
  | Unknown j ->
      j
