...
module Main = struct
  ...
  type input = {phone: string; code: string} [@@deriving yojson {strict= false}]

  type output = {token: string} [@@deriving yojson {strict= false}]

  let call ~phone ~code (client : Hermes.client) : output Lwt.t =
    let params = () in
