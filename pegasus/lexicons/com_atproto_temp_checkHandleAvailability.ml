...
type suggestion = {handle: string; method_: string [@key "method"]}
[@@deriving yojson {strict= false}]

type result_unavailable = {suggestions: suggestion list}
[@@deriving yojson {strict= false}]
...
module Main = struct
  ...
  type output = {handle: string; result_: result_ [@key "result"]}
  [@@deriving yojson {strict= false}]

  let call ~handle ?email ?birth_date (client : Hermes.client) : output Lwt.t =
