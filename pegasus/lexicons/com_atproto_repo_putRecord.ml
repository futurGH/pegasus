(* generated from com.atproto.repo.putRecord *)
...
module Main = struct
  ...
  let call ~repo ~collection ~rkey ?validate ~record ?swap_record ?swap_commit
      (client : Hermes.client) : output Lwt.t =
    let params = () in
    let input =
      Some
        ( {repo; collection; rkey; validate; record; swap_record; swap_commit}
        |> input_to_yojson )
    in
