...
let create_temp_db () =
  ...
  let path =
    Printf.sprintf "/tmp/pegasus_bench_%d_%d.db" (Unix.getpid ()) !temp_db_counter
  in
...
let rec unique_paths n acc =
  if n <= 0 then acc
    ...
    let p = make_path () in
    if List.mem p acc then unique_paths n acc else unique_paths (n - 1) (p :: acc)

let generate_blocks count =
  List.init count (fun _ -> random_block ())
...
let bench_single_block_ops () =
  ...
  in
  print_result r3 ;
  cleanup_temp_db path ; Lwt.return_unit
...
let bench_batch_block_ops () =
  ...
  Lwt_list.iter_s
    (fun batch_size ->
      ...
      let block_map =
        List.fold_left (fun acc (cid, data) -> Block_map.set cid data acc) Block_map.empty blocks
      in
      let%lwt r1 =
...
let bench_bulk_insert_ops () =
  ...
  Lwt_list.iter_s
    (fun size ->
      ...
      in
      print_result r2 ;
      cleanup_temp_db path2 ; Lwt.return_unit )
    sizes
...
let bench_db_mst_ops () =
  ...
  Lwt_list.iter_s
    (fun size ->
      ...
      in
      let%lwt r1 =
        time_it (Printf.sprintf "Mst.of_assoc (db-backed, %d records)" size) (fun () ->
            Mst.of_assoc db kv_pairs >|= fun _ -> () )
      in
      print_result r1 ;
      let%lwt mst = Mst.of_assoc db kv_pairs in
      let%lwt r2 =
        time_it (Printf.sprintf "Mst.build_map (db-backed, %d records)" size) (fun () ->
            Mst.build_map mst >|= fun _ -> () )
      in
      print_result r2 ;
      let%lwt r3 =
        time_it (Printf.sprintf "Mst.leaf_count (db-backed, %d records)" size) (fun () ->
            Mst.leaf_count mst >|= fun _ -> () )
      in
      print_result r3 ;
      cleanup_temp_db path ; Lwt.return_unit )
    sizes
...
let bench_db_mst_incremental () =
  ...
  Lwt_list.iter_s
    (fun (initial_size, add_count) ->
      ...
      let%lwt r1 =
        time_it
          (Printf.sprintf "Mst.add incremental (%d to %d-record tree)" add_count initial_size)
          (fun () ->
            Lwt_list.fold_left_s (fun t (k, v) -> Mst.add t k v) mst add_kv >|= fun _ -> () )
      in
      print_result r1 ;
  ...
  Lwt_list.iter_s
    (fun size ->
      ...
      let%lwt () =
        Lwt_list.iter_s
          (fun (_, cid, data, _) ->
            Mist.Storage.Memory_blockstore.put_block store cid data >|= fun _ -> () )
          records
      in
      let kv_pairs = List.map (fun (path, cid, _, _) -> (path, cid)) records in
      let%lwt mst = Mem_mst.of_assoc store kv_pairs in
      let%lwt r1 =
        time_it (Printf.sprintf "to_blocks_stream + consume (%d records)" size) (fun () ->
            let stream = Mem_mst.to_blocks_stream mst in
            Lwt_seq.fold_left_s (fun count _ -> Lwt.return (count + 1)) 0 stream >|= fun _ -> () )
      in
      print_result r1 ;
      let commit_map = Dag_cbor.String_map.(empty |> add "root" (`Link mst.root)) in
      let commit_block = Dag_cbor.encode (`Map commit_map) in
      let commit_cid = Cid.create Dcbor commit_block in
      let%lwt r2 =
        time_it (Printf.sprintf "Car.blocks_to_stream (%d records)" size) (fun () ->
            ...
            Lwt_seq.fold_left_s (fun acc chunk -> Lwt.return (acc + Bytes.length chunk)) 0 car_stream
            >|= fun _ -> () )
      in
      print_result r2 ;
      Lwt.return_unit )
...
let bench_car_import () =
  ...
  Lwt_list.iter_s
    (fun size ->
      ...
      Printf.printf "  (car size for %d records: %d bytes)\n%!" size (Bytes.length car_bytes) ;
      let%lwt r1 =
        time_it (Printf.sprintf "Car.read_car_stream parse (%d records)" size) (fun () ->
            let stream = Lwt_seq.return car_bytes in
            let%lwt roots, blocks_seq = Car.read_car_stream stream in
            let%lwt _ =
              Lwt_seq.fold_left_s (fun acc (_, _) -> Lwt.return (acc + 1)) 0 blocks_seq
            in
            Lwt.return roots )
      in
      print_result r1 ;
...
let bench_rebuild_mst () =
  ...
  Lwt_list.iter_s
    (fun size ->
      ...
      in
      let%lwt r1 =
        time_it (Printf.sprintf "get_all_record_cids + of_assoc (%d records)" size) (fun () ->
...
let bench_mixed_ops () =
  ...
  let%lwt r1 =
    time_it
      (Printf.sprintf "mixed ops (%d ops)" num_ops)
      (fun () ->
        let rec loop mst i =
...
let bench_db_io_patterns () =
  ...
  let%lwt r1 =
    time_it (Printf.sprintf "sequential read (%d blocks)" size) (fun () ->
        Lwt_list.iter_s (fun cid -> User_store.get_bytes db cid >|= fun _ -> ()) cids )
  in
  print_result r1 ;
  let%lwt r2 =
    time_it (Printf.sprintf "random read (%d blocks)" size) (fun () ->
        Lwt_list.iter_s (fun cid -> User_store.get_bytes db cid >|= fun _ -> ()) shuffled_cids )
  in
  print_result r2 ;
  let batch_size = 50 in
  let batches = List.init (size / batch_size) (fun i ->
      List.filteri (fun j _ -> j >= i * batch_size && j < (i + 1) * batch_size) cids )
  in
  let%lwt r3 =
    time_it (Printf.sprintf "batched read (%d blocks, batch=%d)" size batch_size) (fun () ->
        Lwt_list.iter_s (fun batch -> User_store.get_blocks db batch >|= fun _ -> ()) batches )
  in
  print_result r3 ;
  cleanup_temp_db path ; Lwt.return_unit
