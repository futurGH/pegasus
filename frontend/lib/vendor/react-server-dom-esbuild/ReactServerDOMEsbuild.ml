...
const ReactClientFlight = require("@pedrobslisboa/react-client/flight");
...
function createFromFetch(promise, options) {
  const response = createResponseFromOptions(options);
  promise.then(
    function (r) {
...
type callServer = string -> arg list -> React.element Js.Promise.t

type options = {callServer: callServer}

external createFromReadableStreamImpl :
     Webapi.ReadableStream.t
  -> ?options:options
  -> unit
  -> React.element Js.Promise.t = "createFromReadableStream"

external createFromFetchImpl :
     Fetch.response Js.Promise.t
  -> ?options:options
  -> unit
  -> React.element Js.Promise.t = "createFromFetch"

external createServerReferenceImpl :
     string
  -> callServer
  -> 'encodeFormActionCallback option
  -> 'findSourceMapURLCallback option
  -> string option
  -> 'action = "createServerReference"

external encodeReply : 'arg list -> string Js.Promise.t = "encodeReply"

let callServerRef = (ref None : callServer option ref)

let setCallServer callServer = callServerRef := Some callServer

let getCallServer () = !callServerRef

let createFromReadableStream ?callServer stream : React.element Js.Promise.t =
  match callServer with
  | Some callServer ->
      setCallServer callServer ;
      createFromReadableStreamImpl stream ~options:{callServer} ()
  | None ->
      createFromReadableStreamImpl stream ()

let createFromFetch ?callServer promise =
  match callServer with
  | Some callServer ->
      setCallServer callServer ;
      createFromFetchImpl promise ~options:{callServer} ()
  | None ->
      createFromFetchImpl promise ()

let createServerReference serverReferenceId =
  let callServer =
    match getCallServer () with
    | Some callServer ->
        callServer
    | None ->
        raise
          (Invalid_argument
             "No callServer has been set, you are trying to create a server \
              function without passing callServer to createFromFetch or \
              createFromReadableStream" )
  in
